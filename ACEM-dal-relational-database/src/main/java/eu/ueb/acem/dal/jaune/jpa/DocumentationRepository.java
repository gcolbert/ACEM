...
import eu.ueb.acem.domain.beans.jaune.jpa.DocumentationEntity;
...
public interface DocumentationRepository extends GenericRepository<DocumentationEntity> {
	...
	//@Query(value = "MATCH (:Documentation)<-[:categoryContains]-(c:ResourceCategory) RETURN c")
	@Query("SELECT rc FROM ResourceCategory rc, Documentation r WHERE r MEMBER OF rc.resources")
	Set<ResourceCategoryEntity> getCategories();

	//@Query(value = "MATCH (p:Person)-[:worksForOrganisation]->(:Organisation)-[*0..2]->(:Organisation)-[:possessesResource|:accessesResource|:supportsResource]->(:Documentation)<-[:categoryContains]-(c:ResourceCategory) WHERE id(p)=({personId}) RETURN c")
	...
	//@Query(value = "MATCH (r:Documentation)<-[:categoryContains]-(c:ResourceCategory) WHERE id(c)=({categoryId}) RETURN r")
	@Query("SELECT r FROM Documentation r, ResourceCategory rc WHERE rc.id = :categoryId AND rc MEMBER OF r.categories")
	Set<DocumentationEntity> getEntitiesWithCategory(@Param("categoryId") Long categoryId);

	//@Query(value = "MATCH (p:Person)-[:worksForOrganisation]->(:Organisation)-[*0..2]->(:Organisation)-[:possessesResource|:accessesResource|:supportsResource]->(r:Documentation)<-[:categoryContains]-(c:ResourceCategory) WHERE id(p)=({personId}) AND id(c)=({categoryId}) RETURN r")
	@Query("SELECT DISTINCT (d) FROM Documentation d JOIN d.categories rc "
			+ "WHERE rc.id = :categoryId "
			+ "  AND ("
			+ "        d.id IN ( SELECT e.id FROM Documentation e, Organisation o, Person u "
			+ "              WHERE u.id = :personId "
			+ "                AND o MEMBER OF u.worksForOrganisations "
			+ "                AND (   o = e.organisationPossessingResource "
			+ "                     OR o = e.organisationSupportingResource "
	...
	Set<DocumentationEntity> getResourcesInCategoryForPerson(@Param("categoryId") Long categoryId, @Param("personId") Long personId);
