...
public interface AdministrativeDepartmentRepository extends GenericRepository<AdministrativeDepartmentEntity> {
	...
	// @Query(value =
	// "MATCH (p:Person)-[:worksForOrganisation]->(o:Organisation)-[*0..2]->(:Organisation)-[:possessesResource|:accessesResource|:supportsResource]->(r:Resource)<-[:supportsResource]-(s:AdministrativeDepartment) WHERE id(p)=({personId}) RETURN s")
	@Query("SELECT DISTINCT (ad) FROM AdministrativeDepartment ad JOIN ad.supportedResources d "
