...
public interface PedagogicalSessionRepository extends GenericRepository<PedagogicalSessionEntity> {
	...
	@Query("SELECT ses FROM PedagogicalSession ses, PedagogicalSequence seq WHERE seq.id = :sequenceId AND ses MEMBER OF seq.pedagogicalSessions AND ses NOT IN ( SELECT ses2 FROM PedagogicalSession ses2, PedagogicalSession ses3 WHERE ses3.nextPedagogicalSession = ses2 AND ses3 MEMBER OF seq.pedagogicalSessions )")
