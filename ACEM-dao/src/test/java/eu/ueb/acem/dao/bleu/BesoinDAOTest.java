...
public class BesoinDAOTest extends TestCase {
	...
	public final void t1TestBesoinDAORetrieve() {
		...
		// We retrieve the object from the datastore with its id
		Besoin besoin2 = besoinDAO.retrieve(besoin1.getId());
		assertEquals(besoin1.getNom(), besoin2.getNom());

		// We check that we can retrieve the object from the datastore by its "nom" field
		Besoin besoin3 = besoinDAO.retrieve(besoin1.getNom());
		assertEquals(besoin1.getNom(), besoin3.getNom());
	}
